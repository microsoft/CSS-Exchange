...
function Perform-Comparison($baselineData, $pattern, $baseExoVer) {
    Write-Host "BaselineData - $($baselineData.Keys) $baseExoVer"
    $result= @{}
    $vdirBatches = Get-VdirBatches
    $errFound = $false
    $vdirBatches.Keys | ForEach-Object {
        $vdirs = $vdirBatches[$_]
        $jobs = @()
                ...
                foreach ($f in (Get-ChildItem -Recurse -Path $pdir -File -Include *aspx, *asmx, *asax, *js, *css, *htm, *html)) {
                    ...
                    if ($hash.Hash) {
                        ...
                        foreach ($key in $baselines.Keys) {
                            ...
                            if ($baselines[$key] -and $baselines[$key][$hash.Hash] -ne $null) {
                                $found = $true
                                break;
                            }
                        }
...
function Main() {
    ...
    $exchVersion, $installedVers = Find-InstalledVersions # Get-ExchangeVersion
    ...
    $baselineData = Load-Baseline $installedVers

    $result, $errFound = Perform-Comparison $baselineData $pattern $exchVersion
    Write-Host "Comparison complete. Writing results."
    Write-ScriptResult $result $exchVersion $errFound
}

function Load-FromGitHub($url, $filename) {
    $loaded = $true
...
function PreProcess-Baseline($baselines) {
    $data = @{}
...
function Find-InstalledVersions()
{
    ...
    $server = (Get-ExchangeServer) | ?{$_.Identity.Name -eq (hostname)}
    if ($server.AdminDisplayVersion.Major -eq 14)
    {
        $exchange_version = (Get-Command ExSetup | ForEach {$_.FileVersionInfo}).ProductVersion
    }
    else
    {
    ...
    $vdir_paths | ? { Test-Path $_ } | %{Get-ChildItem -Directory -Path $_} | ?{$VALID_VERSIONS[$_.Name] -eq $true} | %{ $versions[$_.Name] = $true }

    return $exchange_version, $versions.Keys
}

function Get-VdirBatches {
    $grps = @{}
    $i = 0
    $batchSize = 10
...
function Load-Baseline($installed_versions) {
    ...
    foreach ($version in $installed_versions) {
        ...
        $filename = (Join-Path (Get-CurrDir) $filename)
        ...
        if (Test-Path $zip_file) {
            ...
            $loaded_checksum = Load-FromGitHub $checksum_url $checksum_file
            ...
            if ($checksum -ne $zip_file_hash.Hash) {
                ...
                $loaded_zip = Load-FromGitHub $zip_file_url $zip_file
            }
            else {
        ...
        if ($zipfile_uptodate -or $loaded_zip) {
            if (Get-Command Expand-Archive -EA SilentlyContinue) {
                ...
                if (Test-Path  $filename) {
                    del $filename -Confirm:$false -Force -Recurse
                }

                [System.IO.Compression.ZipFile]::ExtractToDirectory($zip_file, $filename)
            }

            $csv_file = Get-ChildItem $filename | Select -First 1 | Select FullName
            $baselines = Get-Content $csv_file.FullName
            $processed_baselines = PreProcess-Baseline $baselines
...
function Write-ScriptResult($result, $exchVersion, $errFound) {
    $tmp_file = Join-Path (Get-CurrDir) ($exchVersion + "_" + "result.xml")
