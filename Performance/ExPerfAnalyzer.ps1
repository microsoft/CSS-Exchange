...
Function Convert-PerformanceCounterSampleObjectToServerPerformanceObjectWithQuickAnalyze {
    ...
    Function Get-FullCounterNameObject {
        ...
        if (($FullCounterSamplePath.Contains("(")) -and
            ($FullCounterSamplePath.Contains("#"))) {
    ...
    foreach ($gPath in $gData) {
        ...
        if ($counterDataObj.CounterType -like "AverageTimer*") {
            ...
            if ($frequency -ne 0 -and
                $numTicksDiff -ne 0 -and
                $numOpsDif -ne 0) {
...
Function Write-QuickSummaryDetails {
    ...
    Write-Verbose("[{0}] : Calling Write-QuickSummaryDetails" -f [DateTime]::Now)
    ...
    $Script:measure_display = Measure-Command {
        ...
        $groupCounterCategory = $ServerObject.CounterData | Group-Object CounterCategory | Where-Object { $_.Name -ne "" -and
            $_.Name -ne "Process" } | Sort-Object Name
        $groupCounterCategoryProcess = $ServerObject.CounterData | Group-Object CounterCategory | Where-Object { $_.Name -eq "Process" }
        $groupCounterCategoryN = $ServerObject.CounterData | Group-Object CounterCategory | Where-Object { $_.Name -eq "" }
        ...
        foreach ($gCategory in $groupCounterCategoryProcess) {
            ...
            foreach ($counterGroup in $gCounterNameObject) {
                Add-Line("{0,-$($strLength_detail+2)}" -f ("\" + $counterGroup.Group[0].ObjectName + "(*)\" + $counterGroup.Group[0].CounterName))
                $TopN = $counterGroup.Group | Where-Object { $_.InstanceName -ne "_total" -and
                    $_.InstanceName -ne "idle" } | Select-Object * -ExpandProperty QuickSummaryStats | Sort-Object Avg -Descending | Select-Object -First $ShowNProcesses
        ...
        Add-Line("================")
        Add-Line("{0,-24} : {1}" -f "Report Generated by", "ExPerfAnalyzer.ps1 ")
...
Function Main {
    ...
    Switch ($PSCmdlet.ParameterSetName) {
        "FileDirectory" {
            ...
            $AllFiles = (Get-ChildItem $PerfmonFileDirectory | Where-Object { $_.Name.EndsWith(".blg") }).VersionInfo.FileName
            ...
            switch ($AllFiles.Count) {
                ...
                1 {
                    ...
                    $rawLocalData = Get-PerformanceDataFromFileLocal -FullPath $AllFiles -Counters (Get-CountersFromXml -xmlCounters $xmlCountersToAnalyze -IncludeWildForServers $true) `
                        -MaxSamples $MaxSamples `
                        -StartTime $StartTime `
                        -EndTime $EndTime
                    ...
                    $displayResults = Write-QuickSummaryDetails -ServerObject $mainObject
                    $displayResults
                    break
                }
        ...
        "SingleFile" {
            ...
            $script:perffromlocalfile = Measure-Command { $rawLocalData = Get-PerformanceDataFromFileLocal -FullPath $PerfmonFile `
                    ...
                    -StartTime $StartTime `
                    -EndTime $EndTime }
            $script:convertTotal = Measure-Command { $mainObject = Convert-PerformanceCounterSampleObjectToServerPerformanceObjectWithQuickAnalyze -RawData $rawLocalData.CounterSamples `
                    -XmlList $xmlCountersToAnalyze `
                    -FileName $rawLocalData.FileName `
                    -ReadTimeSpan $rawLocalData.ReadingFileTime }
            Write-QuickSummaryDetails -ServerObject $mainObject
            break
        }
        "RegisterHandler" {
