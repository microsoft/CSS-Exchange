    ...
    foreach ($directory in $SaveLogs.Keys) {
        ...
        if ($directory -eq "Windows-Logs" -and
            ...
            try {
                ...
                Save-DataInfoToFile -DataIn (Get-EventLog System -After ([DateTime]::Now - $PassedInfo.TimeSpan) -Before ([DateTime]::Now - $PassedInfo.TimeSpan)) -SaveToLocation ("{0}\System" -f $saveLocation) -SaveTextFile $false
