...
Function Get-StoreSubmissionData {
    ...
    if ($entry.count -gt 1) { Write-Warning "Found more than one STOREDRIVER RECEIVE event for this message" }
...
Function Get-MIMEData {
    ...
    if ($entry.count -gt 1) { Write-Error "Found more than one SMTP RECEIVE event for this message" }
