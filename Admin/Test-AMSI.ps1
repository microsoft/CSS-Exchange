    ...
    function CheckServerAMSI {
        ...
        try {
            ...
            if ( $_.Exception.Message -eq "The underlying connection was closed: Could not establish trust relationship for the SSL/TLS secure channel.") {
                ...
                $host.ui.RawUI.ForegroundColor = $currentForegroundColor
                $msgDetectedTimeStamp = "You could find result around $((Get-Date).ToUniversalTime().ToString("M/d/yyy h:mm:ss tt")) UTC"
                ...
                if ( $isServer ) {
                    ...
                    if ( $server.ToLower() -eq $env:COMPUTERNAME.ToLower() ) {
                        ...
                        $remoteExchangePath = (Invoke-Command -ComputerName $server -ScriptBlock { $using:env:ExchangeInstallPath } -ErrorAction SilentlyContinue -ErrorVariable InvokeError)
                    ...
                    if ( Test-Path $HttpRequestFilteringLogFolder -PathType Container ) {
                        ...
                        } while ( ( -not $file ) -and ((Get-Date) -lt $timeout) )
                        ...
                        Write-Warning "We could not access HttpRequestFiltering folder on $server"
                    }
                } else {
                ...
                foreach ($m in $Matches.Values ) {
                    ...
                    if ( $ExchangeServer.ToLower() -eq $env:COMPUTERNAME.ToLower() ) {
                        ...
                        $remoteExchangePath = Invoke-Command -ComputerName $server -ScriptBlock {
                            Write-Host "Remote AMSI Providers detection ($($using:env:COMPUTERNAME)): "
                            ...
                            if ( $path ) {
                                ...
                                if ( $WindowsDefenderPath -like '*Windows Defender*') {
                                    ...
                                    Write-Warning "We did not find Windows Defender"
                                }
                            } else {
                                Write-Warning "We did not find AMSI providers."
                            }
    ...
    function CheckAMSIConfig {
        ...
        if ( $getSO ) {
            ...
            if ( $ExchangeServer.ToLower() -eq $env:COMPUTERNAME.ToLower() ) {
                ...
                $remoteExchangePath = (Invoke-Command -ComputerName $ExchangeServer -ScriptBlock { $using:env:ExchangeInstallPath } -ErrorAction SilentlyContinue -ErrorVariable InvokeError)
