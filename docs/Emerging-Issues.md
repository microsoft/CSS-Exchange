...
The Exchange services in Automatic start-up mode will not start after reboot of the server. The services start successfully if started manually | Exchange 2016 installed on Windows 2012 R2, other versions are not affected| Under investigation | Use the workarounds described in [this article](https://support.microsoft.com/help/5023353) <BR> or use the following PowerShell command to check the Exchange services that are configure to start automatically but have not started and start the services: <BR><BR> `Get-Service -DisplayName "Microsoft Exchange*" | Where-Object {$_.StartType -eq "Automatic" -and $_.Status -ne "Running"} | Start-Service` |
