...
UseAuthorizationCodeFlow | Use this switch parameter to force the Graph API access token to be acquired via the OAuth 2.0 authorization code flow with PKCE instead of the default device code flow. The authorization code flow opens a local browser and requires a redirect listener, so it only works on hosts with a browser and with an application that permits a loopback (`http://localhost`) redirect URI (for example, a custom application passed via `CustomClientId`). Note that the default application does not permit a loopback redirect, so this switch requires a custom application.
